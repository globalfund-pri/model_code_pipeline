...
def run_ic7_report(filename: pathlib.Path) -> Dict:
    """Returns the results generated by the running the Report class. (Also generates that Excel file in a temporary
    location.)"""

    # Save the original parameters file content and restore it after the test
    actual_params_file = get_root_path() / "src/scripts/ic7/shared/parameters.toml"
    test_params_file = path_to_data_for_tests / "parameters_for_ic7_test.toml"

    # Create backup of the original file
    backup_file = actual_params_file.with_suffix('.backup')
    shutil.copy2(actual_params_file, backup_file)

    try:
        # Copy the test parameters into the file that will be used when the report is run
        shutil.copy2(test_params_file, actual_params_file)
        ...
        # Restore the original parameters file
        if backup_file.exists():
            shutil.copy2(backup_file, actual_params_file)
            backup_file.unlink()  # Remove the backup file
...
def test_ic7report(tmpdir):
    ...
    filename = tmpdir / 'test_report_ic7.xlsx'
    run_results = run_ic7_report(filename)
